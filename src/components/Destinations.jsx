    ...
    <section id="destinations" ref={sectionRef} className="py-24 md:py-32 px-6 md:px-16 w-full bg-peakWhite dark:bg-peakDark transition-colors">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 md:mb-16 gap-6">
          <div>
            <h2 className="font-display font-bold text-3xl md:text-6xl text-peakDeep dark:text-peakWhite mb-4">
              {t('dest.title')}
            </h2>
            <p className="font-sans text-peakDeep/60 dark:text-peakWhite/60 max-w-xl text-base md:text-lg">
              {t('dest.desc')}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {destinations.map((dest) => (
            <div 
              key={dest.id} 
              className="destination-card relative h-[480px] md:h-[600px] w-full rounded-2xl overflow-hidden group cursor-pointer"
            >
              <div className="absolute inset-0 z-0 overflow-hidden">
                <img 
                  src={dest.image} 
                  alt={`${dest.title} trekking route in Nepal`}
              ...
              <div className="absolute bottom-0 left-0 w-full p-6 md:p-8 z-20 flex flex-col justify-end">
                <div className="flex justify-between items-center mb-3">
                  <span className="bg-peakGreen/90 text-white text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full">
                    {dest.difficulty}
                  </span>
                  <span className="text-white/90 font-sans font-medium text-sm">
                    {dest.days} {t('dest.days')}
                  </span>
                </div>
                
                <h3 className="text-white font-display text-2xl md:text-3xl font-bold mb-3">
                  {dest.title}
                </h3>
                
                <p className="text-white/70 font-sans text-sm mb-4 line-clamp-2 group-hover:line-clamp-none transition-all">
                  {dest.description}
                </p>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="bg-white text-peakDeep px-5 py-2.5 rounded-full font-sans text-xs font-bold uppercase tracking-wider flex items-center gap-2 hover:bg-peakGreen hover:text-white transition-all"
                  >
