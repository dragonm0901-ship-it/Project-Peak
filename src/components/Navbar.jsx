...
import { Sun, CloudMoon, Menu, X } from 'lucide-react';
    ...
    <div className="fixed top-0 left-0 right-0 z-50 px-4 py-3 w-full">
      <nav className="flex items-center justify-between px-5 py-3 md:px-6 md:py-4 rounded-2xl md:rounded-full transition-all duration-300 w-full max-w-7xl mx-auto bg-white/70 dark:bg-peakDeep/80 backdrop-blur-xl border border-black/5 dark:border-white/10 text-peakDeep dark:text-peakWhite shadow-lg">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          {/* Currency & Language — hidden on mobile, shown in mobile menu */}
          <select 
      ...
      {/* Mobile Menu Drawer */}
      {mobileOpen && (
        <div className="md:hidden mt-2 mx-auto max-w-7xl bg-white/95 dark:bg-peakDeep/95 backdrop-blur-xl rounded-2xl border border-black/5 dark:border-white/10 shadow-xl p-6 flex flex-col gap-4 animate-in">
          {navLinks.map(link => (
            <a 
              key={link.href} 
              href={link.href} 
          ...
          <div className="flex gap-3 pt-2">
            <select 
              className="flex-1 bg-transparent border border-black/10 dark:border-white/15 rounded-lg px-3 py-2 cursor-pointer font-sans text-sm font-semibold text-peakDeep dark:text-peakWhite"
              value={currency}
              onChange={(e) => setCurrency(e.target.value)}
              aria-label="Select currency"
            >
          ...
          <a 
            href="#destinations" 
            className="bg-peakGreen text-white px-6 py-3 rounded-full font-sans uppercase tracking-widest text-sm font-bold text-center hover:shadow-lg transition-all"
            onClick={() => setMobileOpen(false)}
          >
            {t('nav.book')}
          </a>
